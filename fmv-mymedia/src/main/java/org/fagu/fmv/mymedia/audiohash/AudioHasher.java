...
/**
 * See https://github.com/starkdg/JPhashAudio/tree/master/src/main/java/org/phash/phashaudio
 */
public class AudioHasher {
	...
	protected static final int nfilts = 33;

	protected static final double frameDuration = 0.40; /* 0.40 seconds per frame */

	protected static final float freqWidth = 1.06f;

	protected final float[] freqs = {50.0f, 75.0f, 100.0f, 125.0f, 150.0f, 200.0f, 250.0f, 300.0f,
	...
	public AudioHasher(int sr) {
		...
		this.overlap = 31 * framelength / 32;
	...
	/**
	 * convenience function to calculate no. seconds of offset found with distance_ber function
	 * 
	...
	protected double compareBlocks(int[] blocka, int[] toggles, int[] blockb) {
		double result = 0;
	 ...
	 **/
	public AudioHashDistance audiohashDistanceBer(AudioHashInfo hash1, AudioHashInfo hash2) {
		...
		for(int i = 0; i < nc; i++) {
			...
			dist = 1.0 - compareBlocks(hash1.hasharray, hash1.toggles, subhash2);
			if(dist > result.getCs()) {
				result.setCs(dist);
				result.setPos(i);
			}
		}
