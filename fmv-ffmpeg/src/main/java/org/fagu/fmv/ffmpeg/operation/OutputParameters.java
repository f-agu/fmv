...
public class OutputParameters extends IOParameters {
	...
	public OutputParameters(AbstractOperation<?, ?> abstractOperation) {
		...
		outputProcessorFactory = new OutputProcessorFactory() {
			...
			@Override
			public OutputProcessor create(OutputParameters outputParameters, MediaOutput output, FilterNaming filterNaming, int index,
					Require require) {
	...
	public OutputProcessor addOutput(MediaOutput output) {
		OutputProcessor outputProcessor = outputProcessorFactory.create(this, output, operation.getFilterNaming(), operation.countIOEntity(
				OutputProcessor.class), operation.require());
